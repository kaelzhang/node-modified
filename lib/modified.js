...
Modified.prototype._init = function() {
  // The events that will emitted from the source stream(readable stream)
  // not by `modified` instance
  ['pipe', 'unpipe', 'error'].forEach(function(ev) {
    this.on(ev, function(src) {
      if (this.req) {

        // Emit 'pipe' event to request object, 
        // then the headers will be configured
        this.req.emit(ev, src);
        // src.pipe(this.req);
      } else {
...
Modified.prototype._request = function(options, callback) {
  ...
  this._readCacheInfo(options, function(err, info) {
    ...
    // `self.req` is always assigned asynchronously
    ...
    req.on('response', function(res) {
      ...
      ? fs.createReadStream(info.data_file) : req;
    ...
    ['request', 'drain', 'end', 'redirect'].forEach(function(ev) {
      req.on(ev, function() {
        self.emit(ev);
      });
    });

    // Tell the internal instance that `this.req` is ready
    self.emit('_ready');
  });
...
Modified.prototype._readCacheInfo = function(options, callback) {
  if (this.cache) {
    this.cache.read(options, callback);
  } else {
    callback(null, {});
  }
...
// Queue the method which depend on `this.req` before `this.req` is ready
function makeQueuedMethod(name, fn, returnValue, afterReal) {
  return function() {
    var args = arguments;
...
Modified.prototype.write = makeQueuedMethod(
  'write',
  function() {
...
Modified.prototype.end = makeQueuedMethod('end', function(chunk) {
  if (chunk) {
    this.write(chunk);
  }
